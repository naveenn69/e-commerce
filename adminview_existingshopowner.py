...
<head>
    ...
    <title>admin_existviewowner</title>
</head>
...
q = "select * from staff where STATUS='APPROVED'"
cur.execute(q)
res = cur.fetchall()
for i in res:
    print(""" 
       <tbody>
        <tr class="active-row">
            ...
            <td><b>%s</b></td>
            <td><b>%s</b></td>""" % (i[0], i[1], i[2], i[3], i[4],i[6]))
    print("""
          <td><input type="submit" value="DELETE" name="Approve"></td>
            </form>
        </tr>
        </tbody>
        """)
...
q = "select * from staff where STATUS='DELETED'"
cur.execute(q)
res = cur.fetchall()
for i in res:
    print(""" 
...
if Approve != None:
    ...
    q2 = """update staff set STATUS='Deleted' where ID='%s'""" % (ID)
